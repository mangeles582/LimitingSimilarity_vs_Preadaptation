...
abun_traits1%>%
  dplyr::select(species, tpl_species,
         genus, tpl_genus,
         sp, tpl_sp)
...
#*get phylogeny using phylo.maker from V.Phylomaker which uses Smith and Brown 2018

sp_names <- basic_info%>%
  dplyr::select(Specid, Family,
         species, genus,
         sp, sub_sp,
         tpl_family,
         tpl_genus,
...
ggsave("./phylo_circle.pdf",
       plot=phylo_plot, 
       width = 30, height = 30, units="cm",
       dpi=600)

## see also plot style with Newick format in case it is not already
#* https://cran.r-project.org/web/packages/TreeTools/vignettes/load-trees.html
#* https://rdrr.io/cran/TreeTools/man/as.Newick.html
...
#* subset of species with functional data for the dplyr::selected
...
names(table)
unique(is.na(table$tpl_species))

plot_df_list <- list()
plot_list <- unique(table$plot_code)
...
for(i in 1:length(plot_list)) {
  ...
  for(j in 1:length(sp_list)){
    ...
    phylo_w_dist <- weighted.mean(dist_x2$dist,
                                  dist_x2$cover_percent)# also real_cover_percent
...
data.table::fwrite(phylo_dist_all,
                   "../../output/tables/dissim/phylo_distances_new.csv",
                   sep=";", dec=".", col.names=T, row.names=F, quote=F)
