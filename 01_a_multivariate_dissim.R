...
##* 110 from 310 species have data for all the dplyr::selected traits 30% aprox
...
row_sum_all <- sum_all%>%
  ...
  dplyr::select(plot_code, total_plant, complete, level)%>%
...
nrow_sum <- rbind(nrow_sum_plot%>%
                    rename(sample_code=plot_code),
                  nrow_sum_transect%>%
                    rename(sample_code=transect_code))
cover_sum <- rbind(cover_sum_plot%>%
                     rename(sample_code=plot_code),
                   cover_sum_transect%>%
                     rename(sample_code=transect_code))

data.table::fwrite(nrow_sum,
                   "../../output/tables/abundances/nrow_representativeness.csv",
                   sep=";", dec=".", col.names=T, row.names=F, quote=F)


data.table::fwrite(cover_sum,
...
#* trait data for the dplyr::selected functional traits
...
acum_df <- sp_df%>%
  dplyr::select(plot_code, new_cover_percent)%>%
...
#*******************************************************##
#** prepare trait pca with the dplyr::selected set of traits ####
#*******************************************************##
...
# remove Quercus species, check and identify outliers
...
pc1_2 <- factoextra::fviz_pca_var(dudi_trait, col.var = "contrib", repel=T,
                         gradient.cols=c("#00AFBB", "#E7B800", "#FF0000"),
                         title="")

pc1_3 <- factoextra::fviz_pca_var(dudi_trait, axes=c(1,3),
                         col.var = "contrib", repel=T,
                         gradient.cols=c("#00AFBB", "#E7B800", "#FF0000"),
                         title="")

pc2_3 <- factoextra::fviz_pca_var(dudi_trait, axes=c(2,3),
                         col.var = "contrib", repel=T,
                         gradient.cols=c("#00AFBB", "#E7B800", "#FF0000"),
                         title="")

three_pca <- gridExtra::grid.arrange(grobs = list(pc1_2, 
                                                  pc1_3,
                                                  pc2_3),
                                    ncol=2, nrow=2,
                                    widths = unit(c(13,13), "cm"), 
                                    heights = unit(c(13,13),'cm'))
...
sort(unique(join_pca_traits$species))
...
for(i in 1:length(plot_list)) {
  ...
  species_list <- unique(table_plot$species)
  ...
  for(j in 1:length(species_list)){
    ...
    # species_id <- unique(table_plot_sp$species_code)
    
    species_id <- table_plot%>%
      filter(species== species_list[j])%>%
      dplyr::select(sp_code)%>%as.character()
    
    trait_centroid <- COGravity(x=table_plot_sp$Axis1, 
                               y=table_plot_sp$Axis2, 
                               wt=table_plot_sp$cover_percent)#same to real_cover_percent
    ...
    dissimilarity <- GMINdistance(centroid_df_plot[, c("trait_centroid_x",
                                                       "trait_centroid_y")],
                                  centroid_df_plot[, c("Axis1",
                                                       "Axis2")])[1]
    ...
    #   xlim(min(table_plot$Axis1)-2,
    #        max(table_plot$Axis1)+2)+
    #   ylim(min(table_plot$Axis2)-2,
    #        max(table_plot$Axis2)+2)+
    ...
    #          alpha = FALSE
    #   )+
    #   geom_point(data=centroid_df_plot,
    ...
    # ggsave(paste0("../../output/community_diagrams/", plot_list[i], ".png"), 
    #        plot=gg_i, width = 9, height = 9, dpi = 300, units="cm")
    # ggsave(paste0("../../output/community_diagrams/", plot_list[i], ".pdf"),
...
data.table::fwrite(centroid_df,
                   "../../output/tables/dissim/dissimilarity_centroid2d_plot_new.csv",
                   sep=";", dec=".", col.names=T, row.names=F, quote=F)
...
source('niche_required_functions.R')# see also gDistance from rgeos package
join_pca_traits
  ...
  for(j in 1:length(species_list)){
    ...
    if(nrow(table_plot_rest)<1){
      ...
      table_plot_rest <- table_plot_rest%>%
        mutate(dissim=GMINdistance(table_plot_sp[,c("Axis1", 
                                                    "Axis2")],
                                   table_plot_rest[,c("Axis1",
                                                      "Axis2")])$min.dist)
      
      av_dissim <- weighted.mean(table_plot_rest$dissim,
                                 table_plot_rest$cover_percent)#also real_cover_percent
...
data.table::fwrite(centroid_df,
                   "../../output/tables/dissim/dissimilarity_weight_distances2d_plot_new.csv",
                   sep=";", dec=".", col.names=T, row.names=F, quote=F)
...
for(i in 1:length(plot_list)) {
  ...
  for(j in 1:length(species_list)){
    ...
    trait_centroid <- COGravity(x=table_plot_sp$Axis1, 
                                y=table_plot_sp$Axis2, 
                                z=table_plot_sp$Axis3,
                                wt=table_plot_sp$cover_percent)#cover_percent
...
data.table::fwrite(centroid_df,
                   "../../output/tables/dissim/dissimilarity_centroid3d_plot_new.csv",
                   sep=";", dec=".", col.names=T, row.names=F, quote=F)
...
for(i in 1:length(plot_list)) {
  ...
  for(j in 1:length(species_list)){
    ...
    if(nrow(table_plot_rest)<1){
      ...
      av_dissim <- weighted.mean(table_plot_rest$dissim,
                                 table_plot_rest$cover_percent)#real_cover_percent
...
data.table::fwrite(centroid_df,
                   "../../output/tables/dissim/dissimilarity_weight_distances3d_plot_new.csv",
                   sep=";", dec=".", col.names=T, row.names=F, quote=F)
